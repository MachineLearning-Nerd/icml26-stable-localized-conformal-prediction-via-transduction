...
def build_supporting_pages(out_dir, analysis, env):
    ...
    limits = "\n".join([
        ...
        "- Theorems 4.2, 4.6 and 4.7 are universally quantified. The **simulations** of them at the",
        "  paper's own scale corroborate their measurable predictions — an estimated O(n⁻¹) exponent,",
        "  a coverage band, a λ-robustness envelope — but are not proof verification and are not",
        "  claimed to be. The **certificates** are proof-level, and their scope is narrower than the",
        "  theorems: each certifies that the *printed proof* derives the *stated result* from the",
        "  *stated assumptions*, and nothing about whether those assumptions hold for any dataset.",
        "- The Theorem 4.6 certificate is explicitly partial. It covers parts (a) and (c) of Claim 3",
        "  completely and part (b)'s algebra, and it does **not** reach that proof's probabilistic",
        "  core — the `o_p(1)` Hessian consistency, the DKW and Hoeffding pointwise rates, dominated",
        "  convergence, or the fixed-point argument. Those five are named on the Claim 3 page and",
        "  taken on trust; a reader who does not grant them should read C3 as corroborated, not",
        "  proved.",
        "- No certificate says anything about the size of any constant. `C` in Theorem 4.2, `C_L` in",
        "  Lemma 4.4 and `c_d` in Assumption 4.5 are all unspecified, so none of these bounds can be",
        "  falsified at a single parameter value — which is also why Theorem 4.2's simulation tests",
        "  its operative consequence (coverage validity across the authors' full λ grid) rather than",
        "  the bound itself.",
        "- The Theorem 4.7 defect is a defect in the **guarantee**, not a demonstrated failure of the",
        "  method. No run measured here covers less than the printed band; what is shown is that the",
        "  printed band does not follow from the selection rule as defined, and that two stated",
        "  repairs restore it.",
        "",
